<div>
    ...
    @if (count($selectedEntries) > 0)
        <div class="grid md:grid-cols-1 select-action-btns-container gap-2">
            <button wire:click="openCommentModal('reviewSelected')"
    ...
    <div class="flex justify-between flex-wrap items-center">
        ...
        <div class="flex sm:space-x-4 space-x-2 sm:justify-end items-center md:mb-6 mb-4 rtl:space-x-reverse">
            ...
            @can('viewAny', \App\Models\Accounting\JournalEntry::class)
                ...
                <button wire:click="showDownloadJournalEntriesForm"
                    class="btn inline-flex justify-center btn-outline-dark dark:bg-slate-700 dark:text-slate-300 m-1">
                    <iconify-icon class="text-xl ltr:mr-2 rtl:ml-2" icon="line-md:download-loop"></iconify-icon>
                                        ...
                                        <td class="table-td flex justify-between">
                                            ...
                                            <div class="dropstart relative text-right">
                                                ...
                                                <ul
                                                    ...
                                                    @if($entry->comm_profile_payment)
                                                        <li wire:click="downloadCommProfilePayment({{ $entry->id }})">
                                                            <span
                                                                class="hover:bg-slate-900 dark:hover:bg-slate-600 dark:hover:bg-opacity-70 hover:text-white w-full border-b border-b-gray-500 border-opacity-10 px-4 py-2 text-sm dark:text-slate-300  last:mb-0 cursor-pointer first:rounded-t last:rounded-b flex space-x-2 items-center capitalize  rtl:space-x-reverse">
                                                                <iconify-icon icon="material-symbols:download"></iconify-icon>
                                                                <span>Download comm profile payment</span></span>
                                                        </li>
                                                    @endif
                                                    @can('review', $entry)
                                                        @if (!$entry->is_reviewed)
                                                            <li wire:click="openCommentModal('review', {{ $entry->id }})">
                                                                <span
                                                                    class="hover:bg-slate-900 dark:hover:bg-slate-600 dark:hover:bg-opacity-70 hover:text-white w-full border-b border-b-gray-500 border-opacity-10 px-4 py-2 text-sm dark:text-slate-300  last:mb-0 cursor-pointer first:rounded-t last:rounded-b flex space-x-2 items-center capitalize  rtl:space-x-reverse">
                                                    ...
                                                    @can('update', $entry)
                                                        @if (!$entry->revert_entry_id)
                                                            <li wire:click="openCommentModal('revert', {{ $entry->id }})">
                                                                <span
                                                                    class="hover:bg-slate-900 dark:hover:bg-slate-600 dark:hover:bg-opacity-70 hover:text-white w-full border-b border-b-gray-500 border-opacity-10 px-4 py-2 text-sm dark:text-slate-300  last:mb-0 cursor-pointer first:rounded-t last:rounded-b flex space-x-2 items-center capitalize  rtl:space-x-reverse">
                                                                    <iconify-icon
                    ...
                    <div
                        ...
                        <div class="relative bg-white rounded-lg shadow dark:bg-slate-700">
                            ...
                            <div class="p-6 space-y-4">
                                ...
                                @if ($entryInfo->is_reviewed)
                                    <div>
                                        <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Review Comment</p>
                                        <div class="text-sm text-slate-600 dark:text-slate-300 mt-1">
                                            {{ $entryInfo->review_comment ?? 'No review comment added.' }}
                                        </div>
                                    </div>
                                @endif
                ...
                <div
                    ...
                    <div class="relative bg-white rounded-lg shadow dark:bg-slate-700">
                        ...
                        <div
                            ...
                            <button wire:click="hideDownloadJournalEntriesForm" type="button"
                                class="text-slate-400 bg-transparent hover:text-slate-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-slate-600 dark:hover:text-white"
                                data-bs-dismiss="modal">
                                <svg aria-hidden="true" class="w-5 h-5" fill="#ffffff" viewBox="0 0 20 20"
                                    xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div class="p-6 space-y-4">
                            ...
                            <div class="from-group">
                                <div class="input-area">
                                    <label for="journalEntriesFrom" class="form-label">From Date</label>
                                    <input id="journalEntriesFrom" type="date"
                                        class="form-control @error('journalEntriesFrom') !border-danger-500 @enderror"
                                        wire:model.lazy="journalEntriesFrom" autocomplete="off">
                                </div>
                                @error('journalEntriesFrom')
                                    <span
                                        class="font-Inter text-sm text-danger-500 pt-2 inline-block">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="from-group">
                                <div class="input-area">
                                    <label for="journalEntriesTo" class="form-label">To Date</label>
                                    <input id="journalEntriesTo" type="date"
                                        class="form-control @error('journalEntriesTo') !border-danger-500 @enderror"
                                        wire:model.lazy="journalEntriesTo" autocomplete="off">
                                </div>
                                @error('journalEntriesTo')
                                    <span
                                        class="font-Inter text-sm text-danger-500 pt-2 inline-block">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div
                            class="flex items-center justify-end p-6 space-x-2 border-t border-slate-200 rounded-b dark:border-slate-600">
                            <button wire:click="downloadJournalEntries" data-bs-dismiss="modal"
                                class="btn inline-flex justify-center text-white bg-black-500">
                                <span wire:loading.remove wire:target="downloadJournalEntries">Download</span>
                                <iconify-icon class="text-xl spin-slow ltr:mr-2 rtl:ml-2 relative top-[1px]"
                                    wire:loading wire:target="downloadJournalEntries"
                                    icon="line-md:loading-twotone-loop"></iconify-icon>
                            </button>
                        </div>
                    </div>
    ...
    {{-- Comment modal shared by review & revert --}}
    @if ($commentModalAction)
        @php
            $isRevert = $commentModalAction === 'revert';
            $modalTitle = $isRevert ? 'Revert Entry' : 'Review Entry';
        @endphp
        <div class="modal fade fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto show"
            tabindex="-1" aria-labelledby="entry_comment_modal" aria-modal="true" role="dialog" style="display: block;">
                ...
                <div
                    ...
                    <div class="relative bg-white rounded-lg shadow dark:bg-slate-700">
                        ...
                        <div
                            ...
                            <button wire:click="closeCommentModal" type="button"
                                class="text-slate-400 bg-transparent hover:text-slate-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-slate-600 dark:hover:text-white">
                                <svg aria-hidden="true" class="w-5 h-5" fill="#ffffff" viewBox="0 0 20 20"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd"
                        ...
                        <!-- Modal body -->
                        <div class="p-6 space-y-4">
                            @if ($commentModalAction === 'reviewSelected')
                                <p class="text-sm text-slate-600 dark:text-slate-300">
                                    Reviewing {{ count($selectedEntries) }} entries. The comment is saved on each of them.
                                </p>
                            @endif

                            <div class="form-group">
                                <label for="actionComment" class="form-label">
                                    {{ $isRevert ? 'Reversal reason' : 'Review comment' }} (optional)
                                </label>
                                <textarea id="actionComment" rows="3"
                                    class="form-control mt-2 w-full {{ $errors->has('actionComment') ? '!border-danger-500' : '' }}"
                                    wire:model.defer="actionComment"
                                    placeholder="{{ $isRevert ? 'Why is this entry being reverted?' : 'Add a note about this review' }}"></textarea>
                                @error('actionComment')
                                    <span class="font-Inter text-sm text-danger-500 pt-2 inline-block">{{ $message }}</span>
                                @enderror
                                @if ($isRevert)
                                    <p class="text-sm text-slate-500 mt-1">
                                        Kept as the comment of the reversal entry. Left empty, the original entry's
                                        comment is used.
                                    </p>
                                @endif
                            </div>
                        ...
                        <div
                            ...
                            <button wire:click="confirmCommentModal"
                                class="btn inline-flex justify-center {{ $isRevert ? 'btn-danger' : 'btn-primary' }}">
                                <iconify-icon class="text-xl spin-slow ltr:mr-2 rtl:ml-2 relative top-[1px]"
                                    wire:loading wire:target="confirmCommentModal"
                                    icon="line-md:loading-twotone-loop"></iconify-icon>
                                <span wire:loading.remove wire:target="confirmCommentModal">{{ $modalTitle }}</span>
                            </button>
                        </div>
                    </div>
                </div>
