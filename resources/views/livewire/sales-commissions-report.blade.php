        ...
        <div class="flex sm:space-x-4 space-x-2 sm:justify-end items-center md:mb-6 mb-4 rtl:space-x-reverse">
            ...
            <div class="dropdown relative ">
                ...
                <ul
                    ...
                    <li wire:click="toggleCollectionDates">
                        <span
                            class="text-slate-600 dark:text-white block font-Inter font-normal px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-600 cursor-pointer">
                ...
                <div class="card">
                    <header class="card-header cust-card-header noborder" style="display: block;">
                        ...
                        @if ($collection_date_from || $collection_date_to)
                            <button class="btn inline-flex justify-center btn-dark btn-sm">
                                <span wire:click="toggleCollectionDates">
                                    {{ $collection_date_from ? 'Collection From: ' . \Carbon\Carbon::parse($collection_date_from)->format('d/m/Y') : '' }}
                                    {{ $collection_date_from && $collection_date_to ? '-' : '' }}
                                    {{ $collection_date_to ? 'Collection To: ' . \Carbon\Carbon::parse($collection_date_to)->format('d/m/Y') : '' }}
                                    &nbsp;&nbsp;
                                </span>
                                <span wire:click="clearCollectionDates">
                                    <iconify-icon icon="material-symbols:close" width="1.2em"
                                        height="1.2em"></iconify-icon>
                                </span>
                            </button>
                                ...
                                <tbody class="bg-white divide-y divide-slate-100">
                                    @forelse ($commissions as $commission)
                                        <tr>
                                            ...
                                            <td class="table-td">
                                                {{ $commission->from ? ucwords(str_replace('_', ' ', $commission->from)) : 'N/A' }}
                                            </td>
                                            <td class="table-td text-right">
                                                {{ number_format((float) $commission->comm_percentage, 2, '.', ',') }}%
                                            </td>
                                            <td class="table-td text-right">
                            ...
                            <div>
                                @foreach ($modalSelectedProfiles as $profile)
                                    <span class="badge bg-slate-900 text-white capitalize rounded-3xl">
                                        {{ $profile->title }}
                                        <iconify-icon icon="material-symbols:close" width="1.2em"
                                        height="1.2em" wire:click="removeProfile({{ $profile->id }})"></iconify-icon>
                                    </span>
                                @endforeach
                            </div>
                ...
                <div
                    ...
                    <div class="relative bg-white rounded-lg shadow dark:bg-slate-700">
                        <div
                            ...
                            <button wire:click="toggleCollectionDates" type="button"
                                class="text-slate-400 bg-transparent hover:text-slate-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-slate-600 dark:hover:text-white">
                                <svg aria-hidden="true" class="w-5 h-5" fill="#ffffff" viewBox="0 0 20 20"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd"
                        ...
                        <div class="p-6 space-y-4">
                            <div class="grid md:grid-cols-2 gap-4">
                                <div>
                                    <label class="form-label">From</label>
                                    <input type="date" class="form-control"
                                        wire:model.defer="Ecollection_date_from">
                                </div>
                                <div>
                                    <label class="form-label">To</label>
                                    <input type="date" class="form-control"
                                        wire:model.defer="Ecollection_date_to">
                                </div>
                            </div>
                        </div>
                        ...
                        <div
                            class="flex items-center justify-end p-6 space-x-2 border-t border-slate-200 rounded-b dark:border-slate-600">
                            <button wire:click="setCollectionDates"
                                class="btn inline-flex justify-center text-white bg-black-500">
                                <span wire:loading.remove wire:target="setCollectionDates">Submit</span>
                                <iconify-icon class="text-xl spin-slow ltr:mr-2 rtl:ml-2 relative top-[1px]"
                                    wire:loading wire:target="setCollectionDates"
                                    icon="line-md:loading-twotone-loop"></iconify-icon>
                            </button>
                            <button wire:click="clearCollectionDates"
                                class="btn inline-flex justify-center btn-outline-dark">
                                Clear
                            </button>
                        </div>
