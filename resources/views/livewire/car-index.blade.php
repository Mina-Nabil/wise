    ...
    <div class="flex justify-between flex-wrap items-center">
        ...
        <div class="flex sm:space-x-4 space-x-2 sm:justify-end items-center md:mb-6 mb-4 rtl:space-x-reverse">
            @can('create', \App\Models\Cars\Car::class)
                <button wire:click="downloadCarsExport"
                            ...
                            <tbody class="bg-white divide-y divide-slate-100 dark:bg-slate-800 dark:divide-slate-700">
                                ...
                                @foreach ($cars as $car)
                                    <tr>
                                        ...
                                        <td class="table-td hover:bg-slate-200 dark:hover:bg-slate-700 cursor-pointer" @can('update', $car->car_model->brand)
                                        wire:click="openBrand({{ $car->car_model->brand->id }})"
                                        @endcan>
                                            {{ $car->car_model->brand->name }}</td>
                                        <td class="table-td hover:bg-slate-200 dark:hover:bg-slate-700 cursor-pointer" @can('update', $car->car_model)
                                    ...
                                    <li class="inline-block relative top-[3px] text-base font-Inter ">
                                        @if ($editModelField)
                                            <div class="relative">
                                                ...
                                                <button wire:click="saveModelName" class="absolute right-0 top-1/2 -translate-y-1/2 w-9 h-full border-none flex items-center justify-center text-slate-600 dark:text-slate-300">
                                                    <iconify-icon icon="material-symbols:save"></iconify-icon>
                                                </button>
                                            </div>
                                        @else
                    ...
                    <div class="relative bg-white rounded-lg shadow dark:bg-slate-700">
                        ...
                        <div class="p-6 space-y-4">
                            <div class="from-group">
                                <label for="lastName" class="form-label">Cars File (Excel format)</label>
                                <input wire:model="carsFile" type="file" class="form-control w-full "
                                    name="basic" accept=".xls,.xlsx" />
                                @error('carsFile')
                                    <span
                            ...
                            <div class="text-sm text-slate-600 dark:text-slate-400">
                                ...
                                <ul class="list-disc list-inside mt-2">
                                    <li>Column B: Brand name</li>
                                    <li>Column C: Car model name</li>
                                    <li>Column D: Car category</li>
                                    <li>Row 2: Model years (headers)</li>
                                    <li>Rows 3+: Price data for each car model/year combination</li>
                                </ul>
                            </div>
                        </div>
                        <!-- Modal footer -->
                        <div wire:click="uploadCars"
