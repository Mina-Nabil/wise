        ...
        <div class="flex justify-end mb-5">
            <div class="dropdown relative">
                ...
                <ul
                    ...
                    <li>
                        <a wire:click="openSetRenewalOfferSec"
                            class="text-slate-600 dark:text-white block font-Inter font-normal px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-600
                                dark:hover:text-white cursor-pointer">
                            Link to Existing Renewal Offer</a>
                    </li>
                    @endcan
                    @can('update', $soldPolicy)
                    <li>
                                                ...
                                                @foreach ($soldPolicy->sales_comms as $comm)
                                                    <tr>
                                                        ...
                                                        <td class="table-td ">
                                                            ...
                                                            @else
                                                                <span class="badge bg-primary-500 text-white h-auto text-[10px]">Default</span>
                                                            @endif
                                                        </td>
                                                        ...
                                                        @can('update', $comm)
                                                            <td class="table-td ">
                                                                <div class="dropup relative z-10">
                                                                    <button class="inline-flex justify-center items-center"
                                                                        type="button" data-bs-toggle="dropdown"
                                                                        aria-expanded="false">
                                                                        <iconify-icon class="text-xl ltr:ml-2 rtl:mr-2"
                                                                    ...
                                                                    <ul
                                                                        class="dropdown-menu min-w-max absolute text-sm text-slate-700 dark:text-white hidden bg-white dark:bg-slate-700 shadow z-50 float-left overflow-hidden list-none text-left rounded-lg mb-1 m-0 bg-clip-padding border-none">
                                                                        @if ($comm->is_new)
                                                                            <li>
                                                                        ...
                                                                        @if (Auth::user()->is_admin && $comm->can_change_type)
                                                                            <li>
                                                                                <a wire:click="toggleCommType({{ $comm->id }})"
                                                                                    class="hover:bg-slate-900 dark:hover:bg-slate-600 dark:hover:bg-opacity-70 hover:text-white w-full border-b border-b-gray-500 border-opacity-10 px-4 py-2 text-sm dark:text-slate-300  last:mb-0 cursor-pointer first:rounded-t last:rounded-b flex space-x-2 items-center capitalize  rtl:space-x-reverse">
                                                                                    <iconify-icon
                                                                                        icon="lets-icons:target-fill"></iconify-icon>
                                                                                    <span>Change to {{ $comm->is_direct ? 'Default' : 'Direct' }}</span></a>
                                                                            </li>
                                                                        @endif
        ...
        @can('viewCommission', $soldPolicy)
            ...
            <div class="card rounded-md bg-white dark:bg-slate-800  shadow-base mt-5">
                <div class="card-body flex flex-col p-6 active justify-center">
                    <header class="card-header noborder flex justify-between">
                        <h4 class="card-title">
                            Policy Commissions (Expected Total: {{ $soldPolicy->comms_details->sum('amount') }})
                        </h4>
                        @can('updateWiseCommPayments', $soldPolicy)
                ...
                <div
                    ...
                    <div class="relative bg-white rounded-lg shadow dark:bg-slate-700">
                        ...
                        <div
                            class="flex items-center justify-between p-5 border-b rounded-t dark:border-slate-600 bg-black-500">
                            <h3 class="text-xl font-medium text-white dark:text-white capitalize">
                                Link to Existing Renewal Offer
                            </h3>
                            <button wire:click="closeSetRenewalOfferSec" type="button"
                                class="text-slate-400 bg-transparent hover:text-slate-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-slate-600 dark:hover:text-white"
                                data-bs-dismiss="modal">
                                <svg aria-hidden="true" class="w-5 h-5" fill="#ffffff" viewBox="0 0 20 20"
                                    xmlns="http://www.w3.org/2000/svg">
                    ...
                    11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                        ...
                        <div class="p-6 space-y-4">
                            <div class="from-group">
                                <label for="renewalOfferId" class="form-label">Offer ID</label>
                                <input type="number" name="renewalOfferId" placeholder="Enter the Offer ID"
                                    class="form-control mt-2 w-full @error('renewalOfferId') !border-danger-500 @enderror"
                                    wire:model.defer="renewalOfferId">
                                @error('renewalOfferId')
                                    <span
                                        class="font-Inter text-sm text-danger-500 pt-2 inline-block">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="text-sm text-slate-600 dark:text-slate-300">
                                ...
                                <ul class="list-disc list-inside space-y-1">
                                    <li>Mark this policy as renewed</li>
                                    <li>Link the offer to this expiring policy</li>
                                    <li>If the offer has a sold policy, it will be linked back to this policy</li>
                                </ul>
                            </div>
                        </div>
                        <!-- Modal footer -->
                        <div
                            class="flex items-center justify-end p-6 space-x-2 border-t border-slate-200 rounded-b dark:border-slate-600">
                            <button wire:click="closeSetRenewalOfferSec" data-bs-dismiss="modal"
                                class="btn inline-flex justify-center text-slate-600 dark:text-white bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600">
                                Cancel
                            </button>
                            <button wire:click="setRenewalOffer" data-bs-dismiss="modal"
                                class="btn inline-flex justify-center text-white bg-black-500">
                                <span wire:loading.remove wire:target="setRenewalOffer">Link Offer</span>
                                <iconify-icon class="text-xl spin-slow ltr:mr-2 rtl:ml-2 relative top-[1px]"
                                    wire:loading wire:target="setRenewalOffer"
                                    icon="line-md:loading-twotone-loop"></iconify-icon>
                            </button>
                        </div>
                    </div>
