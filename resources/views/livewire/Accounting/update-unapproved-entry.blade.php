        ...
        <div class="card-body">
            <div class="card-text h-full">
                <header class="border-b px-4 pt-4 pb-3 flex items-center border-primary-500">
                    <iconify-icon class="text-3xl inline-block ltr:mr-2 rtl:ml-2 text-primary-500" icon="clarity:update-line"></iconify-icon>
                    <h3 class="card-title mb-0 text-primary-500">Unapproved Journal Entry</h3>
                </header>
                        ...
                        <div class="">
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
                                <div class=" card-body rounded-md bg-[#E5F9FF] dark:bg-slate-800 shadow-base menu-open p-5">
                                    ...
                                    @if ($debit_doc_url)
                                        <button wire:click="downloadDebitDoc" class="btn btn-sm inline-flex justify-center btn-dark"><iconify-icon icon="charm:download" width="1.2em" height="1.2em"></iconify-icon> Download document</button>
                                    @else
                                        <div class="mb-4">
                                            <label for="new_debit_doc_url" class="block text-gray-700 dark:text-gray-300">Document</label>
                                            <input type="file" id="new_debit_doc_url" class="mt-1 block w-full p-2 border rounded-md {{ $errors->has('new_debit_doc_url') ? '!border-danger-500' : 'border-gray-300' }}" wire:model.defer="new_debit_doc_url">
                                            @error('new_debit_doc_url')
                                                <span class="text-red-500 text-sm">{{ $message }}</span>
                                            @enderror
                                        </div>
                                    @endif
                                ...
                                <div class=" card-body rounded-md bg-[#E5F9FF] dark:bg-slate-800 shadow-base menu-open p-5">
                                    ...
                                    @if ($credit_doc_url)
                                        <button wire:click="downloadCreditDoc" class="btn btn-sm inline-flex justify-center btn-dark"><iconify-icon icon="charm:download" width="1.2em" height="1.2em"></iconify-icon> Download doucument</button>
                                    @else
                                        <div class="mb-4">
                                            <label for="new_credit_doc_url" class="block text-gray-700 dark:text-gray-300">Document</label>
                                            <input type="file" id="new_credit_doc_url" class="mt-1 block w-full p-2 border rounded-md {{ $errors->has('new_credit_doc_url') ? '!border-danger-500' : 'border-gray-300' }}" wire:model.defer="new_credit_doc_url">
                                            @error('new_credit_doc_url')
                                                <span class="text-red-500 text-sm">{{ $message }}</span>
                                            @enderror
                                        </div>
                                    @endif
