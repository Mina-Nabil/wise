...
class CustomerMerge extends Component
{
    ...
    public function mount($ids = null)
    {
        ...
        $candidates = Customer::whereIn('id', $this->candidateIds)->get();
    ...
    public function deleteCandidate($id)
    {
        ...
        try {
            ...
            report($e);
            AppLog::error('Customer delete failed', desc: $e->getMessage(), loggable: $candidate);
        ...
        if (count($this->candidateIds) < 2) {
            return redirect()->route('reports.customer-duplicates');
        }
    }
    ...
    public function mergeCustomers()
    {
        ...
        $customer = Customer::find($data['survivor']);
        if (!$customer) {
        ...
        try {
            if ($customer->mergeCustomers($data['sources'], (int) $this->ownerId)) {
                return redirect()->route('reports.customer-duplicates', ['merged' => 1]);
            }
            $this->fail('Unable to merge — you may not have permission, or the profiles were not found.');
        } catch (\Throwable $e) {
            report($e);
            AppLog::error('Customer merge failed', desc: $e->getMessage(), loggable: $customer);
    ...
    {
        $candidates = Customer::whereIn('id', $this->candidateIds)
            ->with('owner')
            ->withCount(['offers', 'soldpolicies', 'phones', 'addresses', 'cars'])
            ->orderBy('id')
            ->get();

        return view('livewire.customer-merge', [
