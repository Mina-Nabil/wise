...
class ClientPayment extends Model
{
    ...
    public function scopeIncludeDue(Builder $query)
    {
        ...
        $query->join('policy_comm_conf', function ($j) {
            ...
            ->selectRaw('DATE_ADD(IF( sold_policies.created_at > sold_policies.start, sold_policies.created_at , sold_policies.start) , INTERVAL `policy_comm_conf.due_penalty` DAY ) as penalty_due')
            ->groupBy('client_payments.id');
    }
