...
class ClientPayment extends Model
{
    ...
    public function scopeIncludeDue(Builder $query)
    {
        return $query->join('sold_policies', 'sold_policies.id', '=', 'client_payments.sold_policy_id')
            ...
            ->selectRaw('(DATE_ADD(IF( sold_policies.created_at > sold_policies.start, sold_policies.created_at , sold_policies.start), INTERVAL policy_comm_conf.due_penalty DAY)) as policy_payment_due')
            ->groupBy('client_payments.id');
    }
